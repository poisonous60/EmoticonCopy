import { db } from "./db";
import { emoticons } from "@shared/schema";

export async function seedDatabase() {
  // Check if database already has data
  const existingEmoticons = await db.select().from(emoticons).limit(1);
  
  if (existingEmoticons.length > 0) {
  ...
  try {
    await db.insert(emoticons).values(sampleEmoticons);
    console.log(`Seeded database with ${sampleEmoticons.length} emoticons`);
  } catch (error) {
